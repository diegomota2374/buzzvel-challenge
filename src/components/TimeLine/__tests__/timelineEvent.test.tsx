...
describe("TimelineEvent Component", () => {
  ...
  test("renders correctly with initial props", () => {
    render(<TimelineEvent event={event} index={0} isVisible={false} />);
  ...
  test("becomes visible when isVisible prop is true", () => {
    render(<TimelineEvent event={event} index={0} isVisible={true} />);
    ...
    const indicatorElement = screen.getByTestId("timeline-indicator-0");
    expect(indicatorElement).toHaveClass("bg-blue-500");
  });
});
